...
const TaskFactory = (
	...
	const project = UI.myProjects.find((storedProject) => {
		if (storedProject.ID === projectID) return storedProject;
	});
	...
	const initializeTaskEventListeners = () => {
		...
		titleElem.addEventListener('click', () => {
			enableEditor(titleElem, titleInput);
		});
		bodyElem.addEventListener('click', () => {
			enableEditor(bodyElem, bodyInput);
		});
		titleInput.addEventListener('keypress', (event) => {
			enableEditorInteraction(event, titleElem, titleInput);
		});
		bodyInput.addEventListener('keypress', (event) => {
			enableEditorInteraction(event, bodyElem, bodyInput);
		});
	};
