...
const ProjectFactory = (titleValue, IDval) => {
	...
	const initEventListeners = () => {
		...
		titleInput.addEventListener('keypress', (event) => {
			enableTitleEditorInteraction(event);
		});
	};

	const makeActive = () => {
		Array.from(nav.children).forEach((project) => {
			project.classList.remove('container-nav-project--active');
		});
	...
	const changeTaskCompletion = (ID) => {
		...
		myTasks.find((task) => {
			// eslint-disable-next-line no-param-reassign
			if (task.ID === ID) task.completion = !task.completion;
		});
	};
	...
	const removeTask = (ID) => {
		// eslint-disable-next-line array-callback-return
		myTasks.find((task) => {
			if (task.ID === ID) myTasks.splice(myTasks.indexOf(task), 1);
		});
		projectTaskCount.textContent = myTasks.length;
		localStorage.projects = JSON.stringify(UI.myProjects);
	};
